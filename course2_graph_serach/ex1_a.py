...
from typing import List, Optional, Dict, Tuple, TypeVar, Callable, Union
...
def main_f() -> None:
    ...
    def time_func(f: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            print(f'{f.__name__} Execution time: {(end - start) / 60} minutes')
            return res
        return wrapper
    ...
    temp_rev, to_nodes_rev, g_graph = read_input_to_graph('./scc.txt', reversed=True)

    @time_func
    def reverse_graph(g: Graph, to_nodes: List[List[Node]]) -> List[List[Node]]:
        '''
    ...
    to_nodes = reverse_graph(temp_rev, to_nodes_rev)
...
if __name__ == '__main__':
    ...
    thread = threading.Thread(target=main_f)
    thread.start()