...
class QuickSort(object):
    ...
    def choose_pivot(self, arr: List[int], left: int, right: int, choice: str = "left") -> None:
        if choice == "left":
            piv = left
        elif choice == "right":
            piv = right - 1
    ...
    def quicksort(self, arr: List[int], left: int, right: int, choice: str = "left") -> Optional[List[int]]:
        if left >= right:
            return None
        self.choose_pivot(arr, left, right, choice=choice)
...
qs = QuickSort()
qs.quicksort(arr, left=0, right=(len(arr)), choice='right')
print(arr)
